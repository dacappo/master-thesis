...
function checkCredentials(user, password, callback) {
  ...
  couch.use('users').get('_find', options, (err, res) => {
    callback(res.docs.length === 1 ? true : false);
  });
}
...
\begin{table}[h]
 ...
 \centering
 \begin{tabular}{lll}
  \textbf{Key} & \textbf{Content} \\ \hline
  \_all\_docs             & \\
  \_bulk\_docs            & key decrement \\
  \_design                & key decrement \\
  \_find                  & key [key ...] \\
  \_index                 & key [key ...] \\
  \_explain               & key second \\
  \_changes               & key timestamp \\
  \_compact               & key start end \\
  \_ensure\_full\_commit  & key value \\
  \_security              & key increment \\
  \_purge                 & key db \\
  \_missing\_revs         & key newkey \\
  \_revs\_diff            & key value [expire] \\
  \_revs\_limit           & key value [expire] \\
  \bottomrule 
 \end{tabular}
 \caption{Redis commands affected by parameter overwrite injection}
 \label{tab:redis_commands_affected}
