...
Within this section, the newly discovered injection attacks against the non-relational MongoDB database are outlined.
...
MongoDb will search for a document with the user property set to \textit{admin} and the password property set to \textit{1234}. Through the expansion the the beforehand injected document or respectively the user property, a matching document is found and the admin login is successful.\\
...
\begin{table}[h] 
 ...
 \begin{tabular}{lll}
  ...
  db.collection.updateOne()         & \textbf{filter} update options \\
  db.collection.updateMany()        & \textbf{filter} update options \\
  db.collection.replaceOne()        & \textbf{filter} replacement options \\\hline
  db.collection.deleteOne()         & \textbf{filter} options \\
  db.collection.deleteMany()        & \textbf{filter} options \\
  db.collection.find()              & \textbf{query} projection \\
  db.collection.findAndModify()     & \textbf{query} options \\
...
This section covers the injection attacks against Redis, that were found in scope of this thesis.
