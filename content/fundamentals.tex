...
In this section an introduction to the relevant security topics of this thesis is given. Therefore, general security goals are introduced ,the current state of NoSQl security is outlined and afterwards, the concept of injection attacks is described.

\subsection{Security Goals}
Information security is a general term comprising a practice for protection of data by defending it against attacks. It involves three major security goals for the underlying data. These goals, known as the CIA security model, are defined as follows:

\begin{description}
\item [Confidentiality] Data has to be protected against any kind of unauthorized disclosure and the authenticity of data sources hast to be ensured.
\item [Integrity] Data hast to be protected from any kind of unauthorized modification and consistency, accuracy as well as trustworthy have to be ensured.
\item [Availability] Data hast to be accessible for authorized entities and a usability as well as timely access hast to be ensured. A delay or denial of access and unauthorized removal of data have to be prevented.
\end{description}

Vulnerabilities like missing encryption, software bugs, weak passwords or inaccurate checks enable an attacker to break one of these goals for the targeted data. Attacks in turn are the act of exploiting vulnerabilities.
