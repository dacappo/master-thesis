...
Similar to other classes of attacks, NoSQL injection aims at one of the three security goals - confidentiality, integrity and availability. In this case, the attack is focused on the database layer of an application. This can be achieved by altering the query behavior, especially with regard to typical CRUD operations. Queries in turn can be influenced through user input. When this input is not properly sanitized for the query context, an attacker might be able to change the semantics of the operation. The user input in turn, derives from requests received from a client. These requests represent the attacker surface for NoSQL injection, since their content can be arbitrary manipulated. A successful attack sends a crafted request, so that the database interprets the query in a way that breaks one of the mentioned security goals. \\
